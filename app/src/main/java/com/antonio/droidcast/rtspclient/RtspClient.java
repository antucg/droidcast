package com.antonio.droidcast.rtspclient;
...
import com.antonio.droidcast.rtspclient.RtspClinet.Socket.RtpSocket;
import com.antonio.droidcast.rtspclient.RtspClinet.Video.H264Stream;
...
public class RtspClient {
  ...
  private void ClientConfig(String host, String address, int port) {
    ...
    if (authorName == null && authorPassword == null) {
      ...
      authorBase64 =
          Base64.encodeToString((authorName + ":" + authorPassword).getBytes(), Base64.NO_WRAP);
    }
  ...
  private void sendRequestSetup() throws IOException {
    ...
    if (matcher.find()) {
      ...
      mSession = mResponse.headers.get("session");
    }
    ...
    if (matcher.find()) {
      Log.d(tag,
          "The client port is:" + matcher.group(1) + " ,the server prot is:" + (isTCPtranslate
      ...
      if (isTCPtranslate) {
        ...
        mRtpSocket =
            new RtpSocket(isTCPtranslate, mParams.rtpPort, mParams.host, mParams.serverPort,
                TRACK_VIDEO);
      }
  ...
  private void sendRequestPlay() throws IOException {
    String request =
        "PLAY rtsp://" + mParams.address + " RTSP/1.0\r\n" + "Range: npt=0.000-\r\n" + addHeaders();
    Log.d(tag, request.substring(0, request.indexOf("\r\n")));
    mOutputStream.write(request.getBytes("UTF-8"));
    Response.parseResponse(mBufferreader);
  }
  ...
  private void sendRequestGetParameter() throws IOException {
    String request = "GET_PARAMETER rtsp://"
        + mParams.address
        + "/"
        + sdpInfo.videoTrack
        + " RTSP/1.0\r\n"
  ...
  private String addHeaders() {
    //String headers = "CSeq: " + (++CSeq) + "\r\n";
    //headers += "User-Agent: " + UserAgent + "\r\n";
    //if (authorBase64 != null) {
    //  headers += "Authorization: Basic " + authorBase64 + "\r\n";
    //}
    //if (mSession != null) {
    //  headers += "Session: " + mSession + "\r\n";
    //}
    //headers += "\r\n";
    return "CSeq: " + (++CSeq) + "\r\n"
  ...
  static class Response {
    ...
    public static final Pattern regexStatus =
        Pattern.compile("RTSP/\\d.\\d (\\d+) .+", Pattern.CASE_INSENSITIVE);
    public static final Pattern regexHeader =
        Pattern.compile("(\\S+): (.+)", Pattern.CASE_INSENSITIVE);
    public static final Pattern regexUDPTransport =
        Pattern.compile("client_port=(\\d+)-\\d+;server_port=(\\d+)-\\d+",
            Pattern.CASE_INSENSITIVE);
    public static final Pattern regexTCPTransport =
        Pattern.compile("client_port=(\\d+)-\\d+;", Pattern.CASE_INSENSITIVE);
    public static final Pattern regexSessionWithTimeout =
        Pattern.compile("(\\S+);timeout=(\\d+)", Pattern.CASE_INSENSITIVE);
    public static final Pattern regexSDPgetTrack1 =
        Pattern.compile("trackID=(\\d+)", Pattern.CASE_INSENSITIVE);
    public static final Pattern regexSDPgetTrack2 =
        Pattern.compile("control:(\\S+)", Pattern.CASE_INSENSITIVE);
    public static final Pattern regexSDPmediadescript =
        Pattern.compile("m=(\\S+) .+", Pattern.CASE_INSENSITIVE);
    public static final Pattern regexSDPpacketizationMode =
        Pattern.compile("packetization-mode=(\\d);", Pattern.CASE_INSENSITIVE);
    public static final Pattern regexSDPspspps =
        Pattern.compile("sprop-parameter-sets=(\\S+),(\\S+)", Pattern.CASE_INSENSITIVE);
    public static final Pattern regexSDPlength =
        Pattern.compile("Content-length: (\\d+)", Pattern.CASE_INSENSITIVE);
    public static final Pattern regexSDPstartFlag =
        ...
        if (line.length() > 3 || Describeflag) {
          ...
          if (Describeflag) {
            ...
            if ((sdpContentLength < sdpHaveReadLength + 2) && (sdpContentLength != 0)) {
              Describeflag = false;
              sdpStartFlag = false;
              Log.d(tag, "The SDP info: " + (sdpInfo.audioTrackFlag ? "have audio info.. "
                  : "haven't the audio info.. ") + ";" + (sdpInfo.audioTrackFlag ? (
                  " the audio track is "
                      + sdpInfo.audioTrack) : ""));
              Log.d(tag, "The SDP info: " + (sdpInfo.videoTrackFlag ? "have video info.. "
                  : "haven't the vedio info..") + (sdpInfo.videoTrackFlag ? (" the video track is "
                  + sdpInfo.videoTrack) : "") + ";" + (sdpInfo.videoTrackFlag ? (
                  " the video SPS is "
                      + sdpInfo.SPS) : "") + ";" + (sdpInfo.videoTrackFlag ? (" the video PPS is "
                  + sdpInfo.PPS) : "") + ";" + (sdpInfo.videoTrackFlag ? (
                  " the video packetization mode is "
                      + sdpInfo.packetizationMode) : ""));
              break;
            }
          }
        } else {
