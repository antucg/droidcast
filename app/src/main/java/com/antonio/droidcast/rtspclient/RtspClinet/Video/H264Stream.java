package com.antonio.droidcast.rtspclient.RtspClinet.Video;
...
import com.antonio.droidcast.rtspclient.RtspClient;
...
public class H264Stream extends VideoStream {
  ...
  private void decodeSPS() {
    ...
    if (profile_idc == 100 || profile_idc == 110 || profile_idc == 122 || profile_idc == 144) {
      int chroma_format_idc = (getUeLen(sps, offset) == 1) ? 0
          : (sps[(offset + getUeLen(sps, offset)) / 8] >> (7 - ((offset + getUeLen(sps, offset))
              % 8)));
      offset += getUeLen(sps, offset);//jump chroma_format_idc
      if (chroma_format_idc == 3) offset++; //jump residual_colour_transform_flag
    ...
    if (pic_order_cnt_type == 0) {
      ...
      int num_ref_frames_inpic_order_cnt_cycle =
          (sps[(offset + getUeLen(sps, offset)) / 8] >> (7 - ((offset + getUeLen(sps, offset))
              % 8)));
  ...
  public int getByteBit(byte[] bytes, int offset, int len) {
    ...
    int lastByteZeroNum =
        ((len % 8 + offset % 8 - 8) > 0) ? (16 - len % 8 - offset % 8) : (8 - len % 8 - offset % 8);
    int data = 0;
  ...
  @Override protected void decodeH264Stream() {
    try {
