...
@SuppressWarnings("unused") public class FFServer implements FFmpegInterface {

  private final Context context;
  private FFmpegExecuteAsyncTask ffmpegExecuteAsyncTask;
  private FFServerLoadLibraryAsyncTask ffServerLoadLibraryAsyncTask;

  private static final long MINIMUM_TIMEOUT = 10 * 1000;
  private long timeout = Long.MAX_VALUE;

  private static FFServer instance = null;

  private FFServer(Context context) {
    this.context = context.getApplicationContext();
    Log.setDEBUG(Util.isDebug(this.context));
  }

  public static FFServer getInstance(Context context) {
    if (instance == null) {
      instance = new FFServer(context);
    }
    return instance;
  }

  @Override public void loadBinary(FFmpegLoadBinaryResponseHandler ffmpegLoadBinaryResponseHandler)
    ...
    if (!TextUtils.isEmpty(cpuArchNameFromAssets)) {
      ffServerLoadLibraryAsyncTask =
          new FFServerLoadLibraryAsyncTask(context, cpuArchNameFromAssets,
              ffmpegLoadBinaryResponseHandler);
      ffServerLoadLibraryAsyncTask.execute();
    } else {
      throw new FFmpegNotSupportedException("Device not supported");
    }
  }

  @Override public void execute(Map<String, String> environvenmentVars, String[] cmd,
    ...
    if (cmd.length != 0) {
      String[] ffmpegBinary = new String[] { FileUtils.getFFServer(context, environvenmentVars) };
      String[] command = concatenate(ffmpegBinary, cmd);
      ffmpegExecuteAsyncTask =
          new FFmpegExecuteAsyncTask(command, timeout, ffmpegExecuteResponseHandler);
  ...
  @Override public String getDeviceFFmpegVersion() throws FFmpegCommandAlreadyRunningException {
    ...
    CommandResult commandResult =
        shellCommand.runWaitFor(new String[] { FileUtils.getFFServer(context), "-version" });
  ...
  @Override public String getLibraryFFmpegVersion() {
  ...
  @Override public boolean killRunningProcesses() {
    return Util.killAsync(ffServerLoadLibraryAsyncTask) || Util.killAsync(ffmpegExecuteAsyncTask);
  }
