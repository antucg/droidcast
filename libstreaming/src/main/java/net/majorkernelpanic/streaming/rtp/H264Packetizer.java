...
public class H264Packetizer extends AbstractPacketizer implements Runnable {
   ...
   */
  @SuppressLint("NewApi") private void send() throws IOException, InterruptedException {
    ...
    // Large NAL unit => Split nal unit
      ...
      while (sum < naluLength) {
        ...
        if ((len = fill(buffer, rtphl + 2,
            naluLength - sum > MAXPACKETSIZE - rtphl - 2 ? MAXPACKETSIZE - rtphl - 2
                : naluLength - sum)) < 0) {
          return;
        }
        sum += len;
